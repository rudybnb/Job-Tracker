...
export function financialTableStatements(): ReadonlyArray<string> {
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    ...
    `CREATE INDEX IF NOT EXISTS idx_phase_assignments_sub_phase_id ON phase_assignments(sub_phase_id);`,
    `CREATE INDEX IF NOT EXISTS idx_phase_assignments_contractor_id ON phase_assignments(contractor_id);`,
